...
class BaseCallsBackend(object):
    def setUp(self, model):
        """Setup backend called when backend is registered."""
        pass
    def tearDown(self, model):
        """Tear down backend callend when backend is unregistered."""
        pass
    
    def _get_url_from_instance(self, instance):
        """Get url from instance dependent on `get_absolute_url` method"""
        if instance and hasattr(instance, 'get_absolute_url'):
            return instance.get_absolute_url()
        return None
    def _get_label_from_instance(self, instance):
        """Get label from instance dependent on `__unicode__` method"""
        if instance and hasattr(instance, '__unicode__'):
            return instance.__unicode__()
        return None
    def _get_user_from_instance(self, instance):
        """Get user from instance dependent on `CALLS_USER_ATTRS` on settings.py"""
        if not instance:
            return None
    ...
    def validate(self, url, label, user_to, user_from):
        """Validation the value and return cleaned values when validate true. Subclass should override this method when custome validation is needed."""
    ...
    def _get_user_name_with_permalink(self, user):
        """Get user name with permalink from user instance"""
    ...
    def get_user_to(self, call):
        """Get user name with permalink from `user_to` attribute of call."""
        return self._get_user_name_with_permalink(call.user_to)
        
    def get_user_from(self, call):
        """Get user name with permalink from `user_from` attribute of call."""
        if call.user_from:
    ...
    def get_message(self, call):
        """Get message from call. Subclass must override this method."""
        raise NotImplementedError
    
class BasicCallsBackend(BaseCallsBackend):
    """Basic calls backend. autodiscover from instance and get message with MESSAGE attribute of backend."""
    ...
    def _post_save_callback(self, sender, instance, created, **kwargs):
        """Callend when instance is saved. Subclass should override this method when controlling creation of call is needed."""
        self.autodiscover(instance=instance, created=created)
    
    def _pre_delete_callback(self, sender, instance, **kwargs):
        """Called when instance is deleted. Subclass should override this method when controlling deletation of call is needed."""
    ...
    def autodiscover(self, instance, created, url=None, label=None, user_to=None, user_from=None, read=None):
        """Automatically discover call from instance. `user_to` argument is required otherwise call never be created."""
    ...
    def get_message(self, call, message=None, user_to=None, user_from=None, label=None):
        """Get message with `MESSAGE` attribute of backend. Override it when you want to configure the message.
